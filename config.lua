...
config.motels = {
	[1] = { -- index name of motel
		...
		door = `3620498182`, -- door hash for MLO type
		rentcoord = vec3(-726.0699, -713.8848, 370.1260),
		coord = vec3(-726.0699, -713.8848, 370.1260), -- center of the motel location
			...
			[1] = { -- COORDINATES FOR GABZ PINKCAGE
				door = { -- Door config requires when using MLO
					[1] = { -- requested by community. supports multiple door models.
						coord = vec3(-726.4253, 720.0388, 374.2243), -- exact coordinates of door
						model = `3620498182` -- model of target door coordinates
					},
					-- [2] = {
				...
				stash = vec3(-732.1699, -719.0670, 370.1129), --  requires when using MLO
				wardrobe = vec3(302.58380126953,-207.71691894531,54.598297119141), --  requires when using MLO
				fridge = vec3(-736.1774, -721.8952, 370.1099), --  requires when using MLO
